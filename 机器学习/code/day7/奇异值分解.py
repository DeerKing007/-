...
from sklearn.decomposition import TruncatedSVD # 降维算法的包
from sklearn.model_selection import train_test_split  # 拆分测试集与训练集
filepath = r"E:\AI145\机器学习\资料·\dating.txt"
X=pandas.read_csv(filepath,usecols=[0,1],sep='\t') # 通用方法，将数据加载为一个二维数组 usecols选择读取哪些列
Y=pandas.read_csv(filepath,usecols=(3,),sep='\t') # 通用方法，将数据加载为一个二维数组 usecols选择读取哪些列
#X=TruncatedSVD(n_components=1).fit_transform(X) # n_components降维到指定维度
#print(X)
